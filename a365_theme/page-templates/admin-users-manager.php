...
/**
 * Template Name: admin-users-manager
 * @package A356
 */
...
?>
    ...
    <table id="a365_users" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th>Id</th>
                <th>Email</th>
    ...
    var user_columns = new Array("email","name","type","sex","address", "phone");
    var user_placeholder = new Array("email","name","type","sex","address", "phone");
    ...
    <script>
        ...
        $('#a365_users tfoot th').each( function () {
            var title = $(this).text();
            $(this).html( '<input type="text" placeholder="Search '+title+'" />' );
        } );

        // DataTable
        pri_table = $('#a365_users').DataTable({
        "scrollX": true,
        "scrollY": 300,
        dom: 'BCfrtlip',
        select: true,
    ...
    $.fn.dataTable.ext.search.push(
        function( settings, data, dataIndex ) {
            if ( settings.nTable.id === 'a365_users' ) {
                var min = new Date(formatDate($('#min').val())).getTime();
                var max = new Date(formatDate($('#max').val())).getTime();
                var age = new Date( formatDate(data[6].split(" ")[0] )).getTime(); // use data for the age column
                // //console.log("min: "+min);
                // //console.log("max: "+max);
                // //console.log("age: "+age);
