...
/**
 * Template Name: admin-theodoicanthiep-report
 * @package A356
 */
get_header('admin');
...
$records = $obj->getRecordsForTheodoicanthiepReport();
?>
    ...
    <div><b>Báo cáo theo dõi hiệu quả can thiệp</b></div> 
    ...
    <table id="a365_theodoicanthiep_report" class="display" cellspacing="0" width="100%">
        <thead>
            <tr>
                <th>Tên bài</th>
                <th>Ngày làm bài</th>
    ...
    <script>
    var intervention_gumsue_table;
    $(document).ready(function() {
        // Setup - add a text input to each footer cell
        $('#a365_theodoicanthiep_report tfoot th').each( function () {
            var title = $(this).text();
            $(this).html( '<input type="text" placeholder="Search '+title+'" />' );
        } );
     
        // DataTable
        intervention_gumsue_table = $('#a365_theodoicanthiep_report').DataTable({
            "ajax": "../theodoicanthiep-report.txt",
            "columns": [
                { 'data' : 'exercise_name' },
                { 'data' : 'begin_at' },
                { 'data' : 'end_at' },
                ...
                buttons: [
                    {
                        ...
                        filename: 'bao_cao_theo_doi_can_thiep_' + gettime(),
                    },
                    {
                        extend: 'copy',
                        exportOptions: {
        ...
        // Apply the search
        intervention_gumsue_table.columns().every( function () {
            var that = this;
    ...
    $.fn.dataTable.ext.search.push(
        function( settings, data, dataIndex ) {
            if ( settings.nTable.id === 'a365_theodoicanthiep_report' ) {
    ...
    $(document).ready(function() {
        ...
        $('#min, #max').keyup( function() {
            intervention_gumsue_table.draw();
        } );
    } );
